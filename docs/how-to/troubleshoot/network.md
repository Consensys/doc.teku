...
despite having
correct firewall and forwarding settings, this could be the cause.
...
autodetection errors, in which case, obtaining the CPU details from `/proc/cpuinfo` on Linux or `/usr/sbin/sysctl -a` on
